...
| The resolved fixture directory exists (`is_dir()` true) but cannot actually be listed — e.g. wrong filesystem permissions in a deploy environment (`GeneralUtility::getFilesInDir()` returns an error string instead of an array on a `scandir()` failure) | Error: "Cannot read fixture directory "...". " | `Command::FAILURE` | No |
...
| A SQL statement in a fixture file throws during execution, and a re-probe (`SELECT 1`) immediately afterwards still succeeds | Error: "Failed to import "...": ..." — exception caught **per file**, that file's transaction is rolled back in full, skip-counter incremented, loop continues to the next file | `Command::SUCCESS` (at end) | Other files: yes; this file: no — any statements from that file that ran before the failing one are rolled back too, not left partially applied |
| A SQL statement in a fixture file throws during execution, **and** the immediate re-probe (`SELECT 1`) also fails — i.e. the connection itself is actually gone, not just the one statement | Error: "Database connection lost during import: ..." | `Command::FAILURE` | No — aborts immediately; files not yet processed are never attempted |
| The re-probe above succeeds (connection is fine) but the subsequent `rollBack()` of the failed file's transaction itself throws | Error: "Rollback failed for "...": ..." | `Command::FAILURE` | No — aborts immediately rather than risking a further inconsistent state |
...
> **Connection-loss detection is a live re-probe, not exception-class matching.** Earlier iterations of this command tried to distinguish "the connection died" from "this file's SQL is bad" by checking the thrown exception's class (`Doctrine\DBAL\Exception\ConnectionException` / `ConnectionLost`). That turned out to be unreliable: DBAL's own MySQL `ExceptionConverter` wraps *per-statement privilege errors* (MySQL 1142 "command denied to user ... for table X", 1143 column access denied) in the very same `ConnectionException` class used for genuine connection failures (access denied at connect time, unknown host, "server has gone away"), so no class-based check can tell them apart — a fixture file failing only because the deploy DB user lacks a `GRANT` on one table would otherwise abort the *entire* import instead of just that file. The command now reacts to *any* exception from a file's statements by immediately re-running a trivial `SELECT 1` on the same connection: if that still succeeds, the connection is fine and only this file is skipped; if it also fails, the connection is genuinely gone and the command aborts with `Command::FAILURE`. This also sidesteps DBAL-major-version differences in exception class hierarchies entirely.
>
> **The command effectively never returns `Command::FAILURE` due to bad fixture SQL on an otherwise-healthy connection.** Only a connectivity failure — checked before any file is processed (a pre-flight `SELECT 1` probe), mid-loop during an actual import (the re-probe described above), or on a subsequent failed rollback — or a base-path/directory-readability resolution failure (bad `EXT:` key, a project-relative `--directory` that escapes the project root, or an unreadable fixture directory) produce a non-zero exit code. A fixture file throwing mid-import for any other reason (a genuine SQL error or a privilege error, confirmed by the live re-probe) is caught per file, that file's transaction is fully rolled back, and the command continues with the next file, ending in a `SUCCESS` exit with a printed skip count. **A CI/deploy pipeline that only checks the exit code will not detect that one or more fixture files were silently and fully skipped.** If you rely on this command in an automated pipeline, also inspect its output for skip counts and per-file error lines — do not trust the exit status alone. Consider `--dry-run` as a pre-flight sanity check (file list and statement counts) in CI, though note it does not validate SQL syntax or execute anything.
>
...
| A fixture file's SQL produced an error but the command reported overall success | Each fixture file's SQL execution runs inside its own transaction with its own try/catch; on any error where an immediate `SELECT 1` re-probe still succeeds (the connection itself is fine — this includes privilege/GRANT errors, e.g. MySQL 1142/1143), that file's transaction is rolled back in full and the skip counter is incremented, per "Behavior & Failure Semantics" above. The command's exit code is not affected by this. | Do not trust the exit code alone. Check the printed "Failed to import "...": ..." error line and the final "Imported X fixture file(s). Skipped Y." summary for a nonzero skip count. Use `--dry-run` to check the file list and statement counts ahead of a real run — it lists files and counts, but does not validate SQL syntax or execute anything, so it cannot substitute for checking the skip count after a real run. If a specific file is consistently skipped, check the deploy database user's `GRANT`s on the table(s) that file writes to. |
| Command exits with `Command::FAILURE` | This has four possible causes: (1) a bad `EXT:` key passed to `--directory` that `GeneralUtility::getFileAbsFileName()` could not resolve, a project-relative `--directory` that escapes the project root, or a resolved fixture directory that exists but cannot be listed (permissions); (2) the pre-flight connectivity probe (`SELECT 1`) fails before any file is processed; (3) a fixture file's SQL throws **and** an immediate re-probe of the connection also fails — i.e. the connection itself is genuinely gone, not just that one statement ("Database connection lost during import: ..."); or (4) the re-probe in (3) succeeds but the subsequent rollback of that file's transaction itself throws ("Rollback failed for "...": ..."). | For (1), verify the extension key uses underscores (not hyphens), that the extension is actually loaded/active, that a project-relative path stays inside the project root, and that the resolved fixture directory has read/execute permission for the user running the command. For (2)–(4), confirm database connectivity and credentials are correct and the default TYPO3 database connection stays reachable for the duration of the import — a connection dropped mid-run (e.g. `wait_timeout`, a DB restart) is surfaced as `Command::FAILURE` rather than silently skipping the remaining files, distinct from a per-file SQL/privilege error (which is skipped, not a `Command::FAILURE` cause). |
